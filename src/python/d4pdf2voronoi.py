...
def main(config):
    ...
    flon = xrin.variables["flon"].values[0, 0, :, :] 
    flat = xrin.variables['flat'].values[0, 0, :, :]
    points = []
    ...
    area_ref.to_file(os.path.join(output_path, 'area_per_d4pdfcell.geojson'), index=False, driver='GeoJSON')
    area_ref.drop(columns=['geometry'], inplace=True)
    area_ref = area_ref.groupby('id').agg({'area': 'sum'}).reset_index()
    area_ref.to_csv(os.path.join(output_path, 'area_per_d4pdfcell.csv'), index=False)
