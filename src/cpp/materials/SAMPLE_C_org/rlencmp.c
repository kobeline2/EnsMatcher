...
void endian_conv4(void *idat)
{
  ...
  c=*(uc+0); *(uc+0)=*(uc+3); *(uc+3)=c;
  c=*(uc+1); *(uc+1)=*(uc+2); *(uc+2)=c;

}

int nbit_unpack(unsigned char din[], int nin, unsigned int dout[],
                  int nout, int nbit)
{

  unsigned int wi, n_b2s;
  int i, bitp, dp;
...
/* -------------------------------------------------------------------------- */
/* decode runlength compress ( nbit data version )                            */
...
/* -------------------------------------------------------------------------- */
{
  ...
  switch(utype) {
  case 1:
    for(j=0; j<k; ++j) *(doc+j) = (unsigned char)*(ww+j);
    break;
  case 2:
    for(j=0; j<k; ++j) *(dos+j) = (short)*(ww+j);
    break;
  case 4:
    for(j=0; j<k; j++) *(doi+j) = (int)*(ww+j);
    break;
  }
