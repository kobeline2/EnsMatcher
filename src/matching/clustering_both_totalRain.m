...
%%
totalCentRain = zeros(nCluster,length(id));
for i = 1:length(id)
    totalCentRain(:,i) = sum(centRain(:,i:length(id):end),2);
end

%% 9.地図に色塗り→動画作成
% colorbarの設定
maxRain = max(totalCentRain,[],"all"); % totalCentRainの最大値
minRain = min(totalCentRain,[],"all"); % totalCentRainの最小値
...
% fig = figure('Position',[500 200 1000 250*ceil(nCluster/nCol)], ...
%              'Visible','off'); % 3列目が幅，4列目が高さ
fig = figure('Units','centimeters', ...
             'Position',[10 5 20 6*ceil(nCluster/nCol)]); % 3列目が幅，4列目が高さ
fig.Color = 'white';
...
gx = geoaxes(t,'Basemap','bluegreen'); % MATLABの緑青地図
    for i = 1:length(id)
        facecolor = color(round((totalCentRain(iCluster,i) ...
                                 -minTickLabels) ...
