...
outMovieFile = fullfile(projectFolder,'Movie',basin,'ensemble', ...
                        ...
                        sprintf('%s_%s_original_onBasin.mp4',basin,initTime));
...
%% rainMatrixの読み込み
rainMatrix = zeros(51,ROW*COL*h);
for mem = 1:51
    tempRainMatrix = readmatrix(fullfile(rainMatrixFolder, ...
                                         sprintf('%s_%s_%03d.dat', ...
                                                 basin,initTime,mem)));
    rainMatrix(mem,:) = reshape(tempRainMatrix,[1,ROW*COL*h]);
end
...
minRain = Inf;
for time = 1:h
    onBasinRain = rainMatrix(:,targetIdx*time);
    tempMaxRain = max(onBasinRain,[],"all");
    if tempMaxRain > maxRain
        maxRain = tempMaxRain; % 最大時間雨量
    end
    tempMinRain = min(onBasinRain,[],"all");
    if tempMinRain < minRain
        minRain = tempMinRain; % 最小時間雨量
    end
end
...
dtInitTime.Format = 'yyyy/MM/dd HH:mm';
nowTime = datetime(targetTime,'InputFormat','yyyyMMddHHmm');
...
for time = 1:h
    ...
    for mem = 1:51
        % gx = geoaxes(tile,'Basemap','GSImap'); % 国土地理院発行の白地図
        gx = geoaxes(t,'Basemap','bluegreen'); % MATLABの緑青地図
        for i = 1:length(targetIdx)
            facecolor = color(round((rainMatrix(mem,ROW*COL*(time-1)+targetIdx(i)) ...
                                     -minTickLabels) ...
